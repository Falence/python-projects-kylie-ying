# 3 ways to concatenate strings
# 
# user = "Falence Lemungoh"
# youtuber = "Kylie Ying"
# 
# print("Hello " + user + ". Please subscribe to " + youtuber)
# print("Hello {}. Please subscribe to {}".format(user, youtuber))
# print(f"Hello {user}. Please subscribe to {youtuber}")

adj = input("Adjective: ")
verb1 = input("Verb: ")
verb2 = input("Verb: ")
famous_person = input("Famous person: ")

madlib = f"Computer programming is so {adj}! It makes me so excited all the time because I love to {verb1}. Stay hydrated and {verb2} like you are {famous_person}!"

print(madlib)